...
def read_frames(vid, frame_queue, video_queue):
    count = 0
    global car
    while True:
        try:
...
def update():
    ...
    new_plates = set()
    for v in vehicles:
        new_plates.add(v['numberPlate'])
        if '-' in v['numberPlate']:
            new_plates.add(v['numberPlate'].replace('-', ''))
...
def start_detection():
    ...
    thread1 = threading.Thread(target=read_frames, args=(vid, frame_queue, video_queue))
