...
const DeleteVehiclePendingModal = ({ setModal, type, vehicleId }) => {
  let [vehicle, setVehicle] = useState({});
  let [loading, setLoading] = useState(false);
  useEffect(() => {
    axios
      .get(`http://localhost:3000/api/vehiclesPending/${vehicleId}`)
      .then((response) => {
  ...
  function handleDelete() {
    setLoading(true);
    axios
      .delete(`http://localhost:3000/api/vehiclesPending/${vehicleId}`)
      .then((res) => {
        console.log(res);
        setLoading(true);
  ...
  function handleApprove() {
    setLoading(true);
    axios
      .post(`http://localhost:3000/api/vehiclesPending/approve/${vehicleId}`)
      .then((res) => {
        console.log(res);
        setLoading(true);
          ...
          <div className="flex flex-col min-h-[120px] mt-6">
            {type == "Delete" ? (
              ...
              <>
                <span className="text-[24px] mb-4">Are you sure?</span>
                <span className=" text-sm text-[#5f5d5d]">
                  Do you really want to approve vehicle{" "}
                  <span className="font-bold">{vehicle?.numberPlate}</span>?
                  This process cannot be undone.
                </span>
              </>
...
export default DeleteVehiclePendingModal;
